...
def seisan_parsing(files):
    event_cat={'ID':[],'DateTime':[],'Latitude':[],'Longitude':[],
               'Depth':[],'mag':[],'mag_type':[],'mag_agency':[]}
    picks_cat={'ID':[],'Station':[],'Component':[],'Phase':[],'Date':[],'Pick_Time':[],
               'Amplitude':[],'Period':[],'Distance':[],'Azimuth':[],'BGS_Mag':[]}
...
#                         azimuth=int(line[76:79])
                    ...
                    picks_cat['Component'].append(line[6:8])
                    picks_cat['Phase'].append('S')
                    picks_cat['Date'].append(origin.date())
                    picks_cat['Pick_Time'].append(pick_time)
                    picks_cat['Amplitude'].append('nan')
                    picks_cat['Period'].append('nan')
#                         print(line[70:75])
#                         try:
#                             distance = float(line[70:75])
#                             print distance
...
#                             print event_id
        ...
        event_cat['DateTime'].append(origin)
...
def remove_duplicates_pd(values,column):
    output = []
    seen = set()
    for value in values:
        # If value has not been encountered yet,
        # ... add it to both list and set.
        if value[column] not in seen:
            output.append(value)
            seen.add(value)
    return output    
